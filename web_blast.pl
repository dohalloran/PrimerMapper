...
while (true)
    ...
    $req = new HTTP::Request GET => "http://www.ncbi.nlm.nih.gov/blast/Blast.cgi?CMD=Get&FORMAT_OBJECT=SearchInfo&RID=$rid&ALIGNMENTS=5&DESCRIPTIONS=5";
    $response = $ua->request($req);
...
$req = new HTTP::Request GET => "http://www.ncbi.nlm.nih.gov/blast/Blast.cgi?CMD=Get&FORMAT_TYPE=Text&RID=$rid&ALIGNMENTS=5&DESCRIPTIONS=5";
$response = $ua->request($req);
