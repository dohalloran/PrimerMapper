...
my $bc = $lf->new_ttk__button( -text    => "Get Sequences",
...
my $gbc = $elf->new_ttk__button( -text    => "Get SNP Sequences",
...
my $dgh = $djh->new_ttk__entry( -width        => 3,
                                -textvariable => \$five_prime_SNP );
$dgh->g_grid( -column => 2, -row => 2, -sticky => "we" );

my $dhi = $djh->new_ttk__entry( -width        => 3,
...
my $dst = $dzs->new_ttk__button(
        ...
        -command => sub {
                calculate_SNP();
                graphics_all_primers_SNP();
                graphics_single_view_SNP();
        }
);
$dst->g_grid( -column => 1, -row => 9, -sticky => "e" );

my $xduv = $dzs->new_ttk__button( -text    => "2: Multiplex PCR dimer scores",
                                  -command => sub { primer_dimer_SNP(); } );
$xduv->g_grid( -column => 4, -row => 9, -sticky => "s" );

my $duv = $dzs->new_ttk__button( -text    => "3: Clean-up (run last)",
...
my $gh = $doh->new_ttk__entry( -width        => 3,
                               -textvariable => \$five_prime_end );
$gh->g_grid( -column => 2, -row => 2, -sticky => "we" );

my $hi = $doh->new_ttk__entry( -width        => 3,
...
my $xxtuz = $mw->new_ttk__button( -text    => "EXIT",
...
my $tuz = $gfd->new_ttk__button( -text    => "BLAST",
...
my $st = $zs->new_ttk__button(
        ...
        -command => sub {
                ...
                graphics_single_view();
        }
);
$st->g_grid( -column => 1, -row => 9, -sticky => "e" );

my $ghuv = $zs->new_ttk__button( -text    => "2: Multiplex PCR dimer scores",
                                 -command => sub { primer_dimer(); } );
$ghuv->g_grid( -column => 4, -row => 9, -sticky => "s" );

my $uv = $zs->new_ttk__button( -text    => "3: Clean-up (run last)",
                               -command => sub { clean_up(); } );
...
######## REVERSE PRIMER
                ...
                for ( my $j = length($sequence) - $three_prime_end ;
                       $j < length($sequence) ;
                       $j += 1 )
                {
                        for ( my $y = $kmer_max_SNP ;
                               $y >= $kmer_min_SNP ;
                               $y-- )
                        {
                                #$kmer = int( rand($kmer_diff) ) + $kmer_min;
                                ...
                                my $percentGC_rounded =
                                  sprintf( "%0.1f", $percent_GC );
                            ...
                            #and print results if statements are matched
                                      ...
                                      && $spec eq "N" )
                                {
                                        open( OLIGOS, ">>$out_image" ) or die;
                                        print OLIGOS
"$j\t$selfie_scoreR\t$primer_start_R\t$j\n";
                                        close(OLIGOS);
                                        push @array_length, $len_uniq;
                                        push @array_name,   $id_uniq;
...
sub calculate_SNP {
        ...
        my $seqio = Bio::SeqIO->new( -file   => $fasta_SNP_NEW,
                                     -format => "fasta", );
...
"$id_SNP\t$i\t$Tmrounded\tF:$_\t$selfie_score\t$percentGCrounded\n";
                                        ...
                                        open( OLIGOS_SNP, ">>$out_image_SNP" )
                                          or die;
                                        print OLIGOS_SNP
"$snp\t$selfie_score\t$i\t$primer_end\n";
                                        close(OLIGOS_SNP);
                                        open( OUTS_SNP, ">>$out_single_SNP" )
                                          or die;
                                        print OUTS_SNP
                                          "$i\t$foo_SNP \t$i\t$primer_end\n";
                                        close(OUTS_SNP);
...
################################################################################################################################################################>>>>>>>>>>>>>>>>>>>>>>>>
                ...
                for ( my $j = $snp + $snp_distance ;
                       $j < $downstream ; $j += 1 )
                {
                        for ( my $c = $kmer_max_SNP ;
                               $c >= $kmer_min_SNP ;
                               $c-- )
                        {
                       #$kmer_SNP = int( rand($kmer_diff_SNP) ) + $kmer_min_SNP;
...
################################################################################################################################################################>>>>>>>>>>>>>>>>>>>>>>>>
                ...
                for ( my $p = $kmer_min_SNP ; $p <= $kmer_max_SNP ; $p++ ) {
                        ...
                        if ( $p <= 36 ) {
                                $Tm3_SNP =
                                  calcTm( $revFA, $DNA_conc_SNP, $salt_SNP );
                        }
                        else {
                                $Tm3_SNP =
...
###################
                ...
                for ( my $u = $kmer_max_SNP ; $u >= $kmer_min_SNP ; $u-- ) {
                        ...
                        my $percentGCroundedAB =
                          sprintf( "%0.1f", $percentGCAB );

                        #calculate Tm
                        if ( $u <= 36 ) {
                                $Tm4_SNP =
                                  calcTm( $_, $DNA_conc_SNP, $salt_SNP );
                        }
                        else {
                                $Tm4_SNP =
...
sub make_approximate {
        ...
        if ( $mismatches_allowed == 0 ) { return $pattern }
                ...
                my $after_match =
                  make_approximate( $rest, $mismatches_allowed );
                if ( $first =~ /[ACGT]/ ) {
                        my $after_miss =
...
"BASED ON IN SILICO AND PCR TESTING, PRIMER DIMER SCORES SHOULD BE LESS THAN OR EQUAL TO 12 \n\n";
        while (<IN>) {
                ...
                if ( $_ =~ m/(.+):(.+)\s(.+):(.+)\n/i ) {
                        ...
                        print OUTER
                          "$1:$2\t$3:$4\tDimer_score: $primer_dimer\n";
                }
        }
        close OUTER;
        close IN;
